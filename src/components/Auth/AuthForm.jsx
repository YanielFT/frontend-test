...
export const AuthForm = () => {
  ...
  const { value: nameValue, onChange: nameChangeHandler } = useInput(
    (value) => value.trim().length > 0
  );

  const { value: passValue, onChange: passChangeHandler } = useInput(
    (value) => value.trim().length > 0
  );
  ...
  useEffect(() => {
    if (userInfo) {
      ...
      dispatch(userLogin({ username: "admin", password: "1234" }));
    }
